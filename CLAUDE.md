...
- **Retention**: `device_data` grows ~20 MB/day/device at a 10 s poll. A task in `bin` (startup + every 6 h) calls `DataStore::downsample_and_prune`: full-resolution rows older than `--retention-days` (default 30, 0 disables) fold into `device_data_hourly` avg/min/max JSON rows (aggregation is shape-preserving over the metrics JSON; intermittent keys like `gen_power` average over present samples only). `get_device_data_range` transparently merges both tables — hourly points carry the avg, timestamped at the bucket start.
...
- The per-port actor opens its serial fd lazily and drops it on transport errors (e.g. EIO after a USB replug), reopening on the next request — no restart needed. Kernel names (`/dev/ttyUSB0`) change across replugs while a stale fd holds the old name, so ports are identified by *spec* (`protocols/src/transport/ports.rs`): `usb-serial:<SN>` when the adapter reports a USB serial number, raw path otherwise (CH340 clones often have no serial; FreeBSD enumeration exposes none). Enumeration returns specs, device configs store them, and the actor re-resolves the spec to the current device node on every reopen; a config holding a raw `ttyUSBn` path will not survive renumbering. A **defective USB-RS485 adapter** is indistinguishable from wrong wiring — if two computers see a dead bus, swap the adapter.
- Time-window registers pack hour in the **low byte**, minute in the high byte. Input reg 5 packs SOC (low) | SOH (high).

- Nothing may pull `serialport` with the `libudev` feature — it breaks aarch64 cross-compilation. There is no direct `serialport` dependency anymore; enumeration goes through `tokio_serial` re-exports (mio-serial pins `serialport` with `default-features = false`), and the sysfs fallback still reports USB serial numbers.
...
- `docs/` holds the committed user-facing setup guide (`docs/SETUP.md` + `docs/setup/` images, linked from README) and local reference material (the gitignored vendor register-map PDF). The original aspirational design specs were removed 2026-07-10 — they never matched the implementation; recover from git history if ever needed. README.md, docs/SETUP.md, and this file are the documentation.
